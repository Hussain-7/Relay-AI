...
import { IconDone, IconInfo, IconThinking, IconTool } from "@/components/icons";
import type { RenderTimelineEntry } from "@/lib/chat-utils";
import { formatToolDisplayName, formatToolRuntimeLabel, formatToolStatusLabel } from "@/lib/chat-utils";

export function ActivityStep({ entry, isLast }: { entry: RenderTimelineEntry; isLast: boolean }) {
    ...
    return (
      <li className="activity-step grid grid-cols-[34px_minmax(0,1fr)] gap-3.5">
        <div className="flex flex-col items-center min-h-full" aria-hidden="true">
          ...
          {!isLast ? (
            <span className="w-px flex-1 mt-2 bg-[linear-gradient(180deg,rgba(255,255,255,0.1),rgba(255,255,255,0.02))]" />
          ) : null}
        </div>
        <div className="min-w-0 pt-px">
          <div className="text-[rgba(245,240,232,0.78)] text-base leading-[1.65] [overflow-wrap:anywhere] break-words whitespace-pre-wrap">
            {entry.text}
          </div>
        </div>
      </li>
  ...
  if (entry.kind === "intermediate") {
    return (
      <li className="activity-step grid grid-cols-[34px_minmax(0,1fr)] gap-3.5">
        ...
        <div className="min-w-0 pt-px">
          <div className="text-[rgba(245,240,232,0.68)] text-[0.92rem] leading-[1.55] [overflow-wrap:anywhere] break-words whitespace-pre-wrap">
            {entry.text}
          </div>
        </div>
      </li>
      ...
      <li className="activity-step grid grid-cols-[34px_minmax(0,1fr)] gap-3.5">
        ...
        <div className="min-w-0 pt-px">
          <div className="flex min-w-0 items-center gap-2.5 flex-wrap">
            <div className="min-w-0 text-[rgba(245,240,232,0.9)] text-[0.96rem] font-medium tracking-[-0.01em]">
              {formatToolDisplayName(entry.title)}
            </div>
            <span
              className={`inline-flex items-center justify-center rounded-full px-[9px] py-1 text-[0.72rem] leading-none whitespace-nowrap ${
            ...
            >
              {formatToolStatusLabel(entry.status)}
            </span>
          </div>
          {taskBrief ? (
            <div className="mt-1 text-[rgba(245,240,232,0.54)] text-[0.82rem] leading-[1.45] [overflow-wrap:anywhere] break-words">
              {taskBrief}
            </div>
          ) : null}
          {entry.runtime ? (
            <div className="mt-1 text-[rgba(245,240,232,0.42)] text-[0.78rem]">
              {formatToolRuntimeLabel(entry.runtime)}
            </div>
          ) : null}
