...
export function SidebarMenuPortal({
  ...
  return createPortal(
    <div
      className="fixed z-100 min-w-[128px] border border-[rgba(255,255,255,0.1)] rounded-[12px] bg-[rgba(42,40,36,0.98)] py-1.5 px-1 shadow-[0_8px_30px_rgba(0,0,0,0.4)] backdrop-blur-[18px]"
    ...
    >
      <button
        type="button"
        className={`${itemClass} text-[rgba(245,240,232,0.82)]`}
        onClick={(e) => {
          e.stopPropagation();
          onToggleStar();
        }}
      ...
      {/* Separator */}
      <div className="my-1 mx-2 h-px bg-[rgba(255,255,255,0.07)]" />

      <button
        type="button"
        className={`${itemClass} text-[rgba(239,150,130,0.9)] hover:bg-[rgba(181,103,69,0.12)] hover:text-[#ffd6c7]`}
        onClick={onDelete}
        disabled={isDeleting}
      >
        <IconTrash />
