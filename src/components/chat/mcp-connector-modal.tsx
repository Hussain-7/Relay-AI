...
function ConnectorCard({
  ...
  return (
    <div
      className={`group relative rounded-[12px] border border-[rgba(255,255,255,0.06)] bg-[rgba(255,255,255,0.02)] px-3.5 py-2.5 transition-all duration-150`}
    >
      <div className="flex items-start gap-3">
        {/* Toggle */}
        <div className="pt-0.5">
          <Toggle size="small" enabled={connector.status === "ACTIVE"} onChange={onToggle} />
        </div>

        {/* Info */}
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-2">
            <span className="text-[rgba(245,240,232,0.92)] text-[0.9rem] font-medium truncate">{connector.name}</span>
            <span className={`inline-block h-1.5 w-1.5 rounded-full shrink-0 ${statusColor(connector.status)}`} />
            <span className="text-[rgba(245,240,232,0.36)] text-[0.7rem] shrink-0">{statusText(connector.status)}</span>
          </div>
          <div className="text-[rgba(245,240,232,0.3)] text-[0.76rem] mt-0.5 truncate">
            {connector.status === "ERROR" && connector.lastError ? connector.lastError : truncateUrl(connector.url)}
          </div>
        </div>
...
function AddConnectorForm({ onBack, onCreated }: { onBack: () => void; onCreated: () => void }) {
  ...
  return (
    <div className="flex flex-col h-full">
      ...
      <div className="flex flex-col gap-3.5">
        <div>
          <label className="block text-[rgba(245,240,232,0.52)] text-[0.76rem] font-medium mb-1.5">Display name</label>
          <input
            type="text"
        ...
        <div>
          <label className="block text-[rgba(245,240,232,0.52)] text-[0.76rem] font-medium mb-1.5">Server URL</label>
          <input
            type="url"
    ...
    <ModalBackdrop onClose={onClose}>
      <ModalPanel>
        ...
        <div className="flex-1 overflow-y-auto px-5 pb-5">
          {view === "add" ? (
            <AddConnectorForm onBack={() => setView("list")} onCreated={() => setView("list")} />
          ) : (
            <>
              {/* Add button */}
              <button
              ...
              {isLoading ? (
                <div className="flex flex-col gap-1.5">
                  {Array.from({ length: 3 }, (_, i) => (
                    <div
                      key={i}
                      className="rounded-[12px] border border-[rgba(255,255,255,0.06)] bg-[rgba(255,255,255,0.02)] px-3.5 py-2.5 animate-pulse"
                    >
                      <div className="flex items-start gap-3">
