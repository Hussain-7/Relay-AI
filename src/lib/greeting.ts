...
export async function generateGreeting(userId: string, firstName?: string): Promise<string> {
  try {
    const now = new Date();
    ...
    const cacheKey = `greeting:${userId}:${hour}`;
    const greeting = await getCached(cacheKey, 3600, async () => {
      ...
      return ai || pickFallback(firstName);
    });

    return greeting ?? pickFallback(firstName);
  } catch {
    return pickFallback(firstName);
  }
}
