...
function parseCliArgs(argv) {
    ...
    const executor = flags.executor === "claude" || flags.executor === "codex"
        ? flags.executor
        : undefined;
...
async function ensureRepoPrepared(args) {
    ...
    const cloneResult = await runCommand(`mkdir -p /workspace && if [ -d ${shellEscape(args.workingDir)}/.git ]; then git -C ${shellEscape(args.workingDir)} fetch --all --prune; else git clone ${shellEscape(cloneUrl)} ${shellEscape(args.workingDir)}; fi`, "/", 10 * 60_000);
    ...
    if (cloneResult.exitCode !== 0) {
        throw new Error(`Repository clone/fetch failed with code ${cloneResult.exitCode}`);
    }
    const checkoutBaseResult = await runCommand(`git fetch origin ${shellEscape(baseBranch)} && git checkout ${shellEscape(baseBranch)}`, args.workingDir, 120_000);
    const checkoutWorkingResult = await runCommand(`git checkout -B ${shellEscape(workingBranch)}`, args.workingDir, 60_000);
...
export function describeRunnerBootstrap(options) {
    return `Runner bootstrap for ${options.runId} with tools: ${DEFAULT_TOOLSET.join(", ")}`;
}
export async function runRunnerWithOptions(options) {
...
async function runRunner(args) {
    ...
    try {
        ...
        if (args.executor && args.prompt) {
            ...
            const result = await runCommand(command, args.workingDir, args.maxMinutes * 60_000);
            const gitStatus = await runCommand("git status --short", args.workingDir, 60_000);
            delegateSummary = {
                executor: args.executor,
                command,
                result,
            ...
            if (result.exitCode !== 0) {
                throw new Error(`${args.executor} command exited with code ${result.exitCode}`);
            }
        }
        ...
        const completedPayload = {
            ...
            diffStat,
            summary: delegateSummary?.summary ??
